...
from mantid.simpleapi import CreateSingleValuedWorkspace, LoadCIF, mtd
...
class FourCircle:
    ...
    def generate_reflection_table(self, cif_file,
        ...
        table = np.column_stack([*hkls.T, ds, F2s, two_theta, omega, chi, phi])

        if max_reflections is not None:
            if satellite:
                sort = np.argsort(two_theta)[:max_reflections]
            else:
                sort = np.argsort(F2s)[::-1][:max_reflections]
            return table[sort]
        else:
            return table
